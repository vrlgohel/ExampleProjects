...
public class SoapMtomClient {
    ...
    public static void main(String[] args) throws Exception {
     //   String port = System.getProperty("org.switchyard.component.soap.client.port", "8080");
        Image image = sendMessage("http://localhost:8080/soap-mtom/ImageServiceService?wsdl");
     ...
     public static Image sendMessage(String wsdl) throws Exception {
        ...
        URL fileURL =  Thread.currentThread().getContextClassLoader().getResource("switchyard_icon.jpeg");
        File aFile = new File(new URI(fileURL.toString()));
        long fileSize = aFile.length();

        byte[] param;
        param= new byte[(int) fileSize];
        InputStream in = fileURL.openStream();
        int len = in.read(param);
        while (len < fileSize) {
            len += in.read(param, len, (int) (fileSize - len));
        }
